...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleConstraintViolationException(ConstraintViolationException e) {
        List<String> errors = e.getConstraintViolations().stream()
                ...
                .toList();
        String message = String.format("Exception %s: %s", e.getClass(), errors);
        log.warn(message);
        return new ResponseEntity<>(new ApiError(HttpStatus.BAD_REQUEST, "Validation errors", message),
    ...
    public ResponseEntity<ApiError> handleEntityNotFoundException(EntityNotFoundException e) {
        String message = String.format("Exception %s: %s", e.getClass(), e.getMessage());
        log.warn(message);
        return new ResponseEntity<>(new ApiError(HttpStatus.I_AM_A_TEAPOT, message,
