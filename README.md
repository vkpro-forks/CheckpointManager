...
- **`active` (желтый)**: Активный пропуск, ожидается въезд/вход в ближайшее время.
- **`delayed` (оранжевый)**: Активный пропуск, но въезд/вход не ожидается в ближайшее время.
- **`cancelled` (голубой)**: Пропуск отменен и не может быть использован для въезда/входа.
- **`outdated` (серый)**: Срок действия пропуска истек, и пересечений не было.
- **`warning` (красный)**: Въезд/вход по пропуску состоялся, но срок действия истек и выезд/выход не был зафиксирован.
- **`completed` (зеленый)**: Пропуск полностью использован, по нему были и въезд/вход, и выезд/выход.

<div align="center">
<img alt="Жизненный цикл пропуска" height="600" src="https://github.com/AnvilCoder/CheckpointManager/assets/124284597/c0850916-ad1f-4491-a062-660dcc77a5ea" width="900"/>
</div>
